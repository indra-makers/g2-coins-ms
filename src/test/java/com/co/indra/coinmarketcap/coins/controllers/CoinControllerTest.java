...
public class CoinControllerTest {
    ...
    @Test
    @Sql("/testdata/get_coin_api_external.sql")
    public void getCoinAPIHappyPath() throws Exception {
        CoinApiResponse mockedBody = new CoinApiResponse(new CoinApi("bitcoin", 1, "BTC", "Bitcoin", 19056412.000, 21000000.000, 578381201943.855, 11833701208.211, 30351.002, 2.4318, 29926.389, "https://blockchain.info/"), 12344.654);
        ResponseEntity<CoinApiResponse> entity = new ResponseEntity(mockedBody, HttpStatus.OK);
        Mockito.when(restTemplate.getForEntity(Mockito.anyString(), Mockito.<Class<CoinApiResponse>>any())).thenReturn(entity);
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(Routes.COINS_PATH+Routes.ID_SYMBOLCOIN_PATH, "BTC").contentType(MediaType.APPLICATION_JSON);
        MockHttpServletResponse response = mockMvc.perform(request).andReturn().getResponse();
        Assertions.assertEquals(200, response.getStatus());
    ...
    @Test
    public void getCoinAPINotFoundPath() throws Exception {
        ResponseEntity<CoinApiResponse> entity = new ResponseEntity(HttpStatus.NOT_FOUND);
        Mockito.when(restTemplate.getForEntity(Mockito.anyString(), Mockito.<Class<CoinApiResponse>>any())).thenReturn(entity);
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(Routes.COINS_PATH+Routes.ID_SYMBOLCOIN_PATH, "BTU").contentType(MediaType.APPLICATION_JSON);
        MockHttpServletResponse response = mockMvc.perform(request).andReturn().getResponse();
    ...
    @Test
    public void getCoinAPIBadPath() throws Exception {
        CoinApiResponse mockedBody = new CoinApiResponse(new CoinApi("bitcoin", 1, "BTC", "Bitcoin", 19056412.000, 21000000.000, 578381201943.855, 11833701208.211, 30351.002, 2.4318, 29926.389, "https://blockchain.info/"), 12344.654);
        ResponseEntity<CoinApiResponse> entity = new ResponseEntity(mockedBody, HttpStatus.OK);
        Mockito.when(restTemplate.getForEntity(Mockito.anyString(), Mockito.<Class<CoinApiResponse>>any())).thenReturn(entity);
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(Routes.COINS_PATH+Routes.ID_SYMBOLCOIN_PATH, 12).contentType(MediaType.APPLICATION_JSON);
        MockHttpServletResponse response = mockMvc.perform(request).andReturn().getResponse();
        Assertions.assertEquals(404, response.getStatus());
        String textResponse = response.getContentAsString();
        ErrorResponse error = objectMapper.readValue(textResponse, ErrorResponse.class);
